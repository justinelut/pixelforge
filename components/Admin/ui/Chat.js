...
export default function Chat({ projectid, userid, usertype }) {
               ...
               {isLoading ? ("Loading") : (
                  data.data.docs.map(message => (
                     <div className="chat-message">
                        <div className={`${message.usertype == 'client' ? 'flex items-end' : 'flex items-end justify-end'}`}>
                           <div className={`flex flex-col space-y-2 text-xs max-w-xs mx-2 ${message.usertype == 'client' ? 'order-2 items-start' : 'order-1 items-end'}`}>
                              <div><span className={`px-4 py-2 rounded-lg inline-block text-xl ${message.usertype == 'client' ? 'rounded-bl-none bg-gray-300 text-gray-600' : 'rounded-br-none bg-blue-600 text-white'}`}>{message.message}</span></div>
                           </div>
                        </div>
                     </div>
                  ))
